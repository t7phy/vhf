...
def nielsen_rust(n, m, x):
    # Run the compiled Rust example and capture output
    cmd = ["cargo", "run", "--quiet", "--example", "verify_nielsen", "--", str(n), str(m), str(x)]
    result = subprocess.check_output(cmd).decode().strip()
    re, im = map(float, result.split(','))
    return complex(re, im)

def run_tests():
    # Test cases: (n, m, x)
    # Testing different branches: x < -1, 0 < x < 0.5, 0.5 < x < 1, x > 2
    test_x_values = [-2.5, -0.5, 0.1, 0.4, 0.6, 0.8, 1.0, 1.5, 3.0]
    # test_x_values = []
    # for i in range(0, 101):
    #     test_x_values.append(i * 0.01)
    
    print(f"{'N':>2} {'M':>2} {'X':>5} | {'Diff':>10}")
    print("-" * 30)
    ...
    for n in range(1, 5):
        for m in range(1, 5):
            ...
            for x in test_x_values:
                # try:
                #     res_py = nielsen_py(n, m, x)
                #     res_rs = nielsen_rust(n, m, x)
                    
                #     diff = abs(res_py - res_rs)
                    
                #     status = "✅" if diff < 1e-12 else "❌"
                #     print(f"{n:2} {m:2} {x:5.1f} | {diff:10.2e} {status}")
                # except Exception as e:
                #     print(f"{n:2} {m:2} {x:5.1f} | Error: {e}")
                try:
                    res_py = nielsen_py(n, m, x)
                    res_rs = nielsen_rust(n, m, x)
                    
                    # Calculate absolute difference
                    abs_diff = abs(res_py - res_rs)
                    
                    # Calculate relative difference
                    # We use max(abs(res_py), 1e-15) to avoid division by zero
                    rel_diff = abs_diff / max(abs(res_py), 1e-15)
                    
                    # Status based on relative tolerance (e.g., 1e-12)
                    status = "✅" if rel_diff < 1e-12 else "❌"
                    
                    print(f"{n:2} {m:2} {x:5.1f} | Abs: {abs_diff:8.2e} | Rel: {rel_diff:8.2e} {status}")
                except Exception as e:
                    print(f"{n:2} {m:2} {x:5.1f} | Error: {e}")
