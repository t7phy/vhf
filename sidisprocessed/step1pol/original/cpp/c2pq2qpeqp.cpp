                  ...
                  double Q, double muR, double muF, double muA)
{
  ...
  if (cx == "R" && cz == "D")
  {
    ...
    res = 9 * CF + 3 * LMUF * CF + 5. / 4. * pow(LMUF, 2) * CF - 9 * x * CF - 3 * x * LMUF * CF - 5. / 4. * x * pow(LMUF, 2) * CF - 1. / 2. * pow(pi, 2) * CF - 1. / 6. * pow(pi, 2) * LMUF * CF + 1. / 2. * pow(pi, 2) * x * CF - 1. / 6. * pow(pi, 2) * x * LMUF * CF + 25. / 4. * ln(x) * CF + 3 * ln(x) * LMUF * CF + 1. / 2. * ln(x) * pow(LMUF, 2) * CF - 3. / 4. * ln(x) * x * CF - 3 * ln(x) * x * LMUF * CF + 1. / 2. * ln(x) * x * pow(LMUF, 2) * CF - 1. / 3. * ln(x) * pow(pi, 2) * CF - 1. / 3. * ln(x) * pow(pi, 2) * x * CF + 17. / 8. * pow(ln(x), 2) * CF + pow(ln(x), 2) * LMUF * CF - 15. / 8. * pow(ln(x), 2) * x * CF + pow(ln(x), 2) * x * LMUF * CF + 5. / 12. * pow(ln(x), 3) * CF + 5. / 12. * pow(ln(x), 3) * x * CF - 5. / 2. * ln(x) * ln(omx) * CF + 5. / 2. * ln(x) * ln(omx) * x * CF - 3. / 2. * ln(x) * pow(ln(omx), 2) * CF - 3. / 2. * ln(x) * pow(ln(omx), 2) * x * CF +
          ln(x) * Li2(x) * CF + ln(x) * Li2(x) * x * CF - 3 * ln(omx) * CF - 5. / 2. * ln(omx) * LMUF * CF + 3 * ln(omx) * x * CF + 5. / 2. * ln(omx) * x * LMUF * CF + 1. / 3. * ln(omx) * pow(pi, 2) * CF + 1. / 3. * ln(omx) * pow(pi, 2) * x * CF + 5. / 4. * pow(ln(omx), 2) * CF - 5. / 4. * pow(ln(omx), 2) * x * CF - ln(omx) * Li2(1 - x) * CF - ln(omx) * Li2(1 - x) * x * CF - 2 * ln(omx) * Li2(x) * CF - 2 * ln(omx) * Li2(x) * x * CF - Li3(1 - x) * CF - Li3(1 - x) * x * CF + 1. / 2. * Li2(x) * CF + Li2(x) * LMUF * CF - 1. / 2. * Li2(x) * x * CF + Li2(x) * x * LMUF * CF;
  ...
  if (cx == "R" && cz == "0")
  {
    ...
    double opx = 1. + x;
    double op6xpxsq = 1. + 6. * x + x * x;
    res = -3 * CF - 5. / 2. * LMUF * CF + 3 * x * CF + 5. / 2. * x * LMUF * CF + 1. / 6. * pow(pi, 2) * CF + 1. / 6. * pow(pi, 2) * x * CF - 3 * ln(x) * CF - ln(x) * LMUF * CF + 3 * ln(x) * x * CF - ln(x) * x * LMUF * CF - pow(ln(x), 2) * CF - pow(ln(x), 2) * x * CF + 5. / 2. * ln(omx) * CF - 5. / 2. * ln(omx) * x * CF - Li2(x) * CF - Li2(x) * x * CF;
