...
pub fn r_00(x: f64, Q2: f64, pid: f64, _nf: f64, _var: i8) -> f64 {
    ...
    if below_threshold(x, Q2, m2) {
        ...
        res = (beta*pi*rho*rhoq*(pow(rho,2) + pow(rhoq,2) + rho*rhoq*(6. + rhoq)))/(2.*pow(rho - rhoq,3)) + (pi*rho*rhoq*(2.*pow(rhoq,2) + 2.*rho*pow(rhoq,2) + pow(rho,2)*(2. - (-4. + rhoq)*rhoq))*chi.ln())/(4.*pow(rho - rhoq,3));
        res *= prefac / x;
    }
    res
}
