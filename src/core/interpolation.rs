...
pub struct BasisCache {
    pub xgrid: Vec<f64>,
    pub functions: Vec<Vec<BasisPiece>>, // Replaces HashMap<String, ...>
    pub mode_log: bool,
}
...
pub fn compute_basis_functions(xgrid: Vec<f64>, poly_deg: usize, mode_log: bool) -> BasisCache {
    let xgrid_transformed = if mode_log {
        xgrid.iter().map(|x| x.ln()).collect()
    } else {
        xgrid
    };
    ...
    let mut functions = Vec::with_capacity(num_x);
    for i in 0..num_x {
        functions.push(basis_function_as_list(i, &xgrid_transformed, &blocks_index));
    }

    BasisCache {
        xgrid: xgrid_transformed,
        functions,
        mode_log,
    }
}

pub fn interpolator(ev_point: f64, cache: &BasisCache) -> Vec<f64> {
    let ev = if cache.mode_log { ev_point.ln() } else { ev_point };
    let num_basis = cache.functions.len();
    let mut res_list = vec![0.0; num_basis];

    // OPTIMIZATION 1: Binary search to find the active grid interval
    // We only need to check pieces that overlap with this interval.
    // However, since we iterate basis functions, we can use the 'break' optimization 
    // combined with Horner's method for maximum speed.
    
    for (i, pieces) in cache.functions.iter().enumerate() {
        for piece in pieces {
            if ev > piece.xmin && ev <= piece.xmax {
                // OPTIMIZATION 2: Horner's Method with Fused Multiply-Add (FMA)
                if let Some((&last, rest)) = piece.coeffs.split_last() {
                    let val = rest.iter().rev().fold(last, |acc, &c| acc.mul_add(ev, c));
                    res_list[i] = val;
                }
                break; // Found the active piece for this basis function
            }
        }
    }
    res_list
...
pub fn point_interpolator(ev_point: f64, cache: &BasisCache, i: usize) -> f64 {
    let ev = if cache.mode_log { ev_point.ln() } else { ev_point };
    
    // Direct index access (O(1)) instead of HashMap lookup
    if i < cache.functions.len() {
        for piece in &cache.functions[i] {
            if ev > piece.xmin && ev <= piece.xmax {
                if let Some((&last, rest)) = piece.coeffs.split_last() {
                    return rest.iter().rev().fold(last, |acc, &c| acc.mul_add(ev, c));
                }
            }
        }
    }
