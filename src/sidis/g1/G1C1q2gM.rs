...
fn RG_RG_000(x: f64, z: f64, NF: f64) -> f64 {
    ...
    if z != x && z != 1. - x {
        let tmp: f64 = 1. / 2. * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * pow(z, -1) * NQCD) + (- pow(NQCD, -1)) + NQCD + z * pow(NQCD, -1) + (- z * NQCD) + 1. / 2. * x * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * x * pow(z, -1) * NQCD) + (- x * pow(NQCD, -1)) + x * NQCD;
        res += tmp;
    }
