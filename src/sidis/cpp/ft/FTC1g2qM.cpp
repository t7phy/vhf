...
double RG_DL_000(double x, double z, double NF) {
    double res = 0.0;
    res = x + (- pow(x, 2)) + 1. / 2. * ln(1 - x) + (- ln(1 - x) * x) + ln(1 - x) * pow(x, 2) + (- 1. / 2. * ln(x)) + ln(x) * x + (- ln(x) * pow(x, 2));
    return res;
}

double RG_DL_010(double x, double z, double NF) {
    double res = 0.0;
    res = (- lmuf) + 2 * lmuf * x + (- 2 * lmuf * pow(x, 2));
    return res;
}
...
double RG_RG_000(double x, double z, double NF) {
    ...
    if (z != x && z != 1. - x) {
        double tmp = 0.0;
        tmp = (-1) + 1. / 2. * pow(z, -1) + (- x * pow(z, -1)) + 2 * x + pow(x, 2) * pow(z, -1) + (- 2 * pow(x, 2));
        res += tmp;
    }
