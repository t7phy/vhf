...
double DL_RG_000(double x, double z, double NF) {
    double res = 0.0;
    res = 1. / 4. * z * pow(NQCD, -1) + (- 1. / 4. * z * NQCD) + 1. / 2. * ln(1 - z) * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * ln(1 - z) * pow(z, -1) * NQCD) + (- 1. / 2. * ln(1 - z) * pow(NQCD, -1)) + 1. / 2. * ln(1 - z) * NQCD + 1. / 4. * ln(1 - z) * z * pow(NQCD, -1) + (- 1. / 4. * ln(1 - z) * z * NQCD) + 1. / 2. * ln(z) * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * ln(z) * pow(z, -1) * NQCD) + (- 1. / 2. * ln(z) * pow(NQCD, -1)) + 1. / 2. * ln(z) * NQCD + 1. / 4. * ln(z) * z * pow(NQCD, -1) + (- 1. / 4. * ln(z) * z * NQCD);
    return res;
}

double DL_RG_001(double x, double z, double NF) {
    double res = 0.0;
    res = (- lmua * pow(z, -1) * pow(NQCD, -1)) + lmua * pow(z, -1) * NQCD + lmua * pow(NQCD, -1) + (- lmua * NQCD) + (- 1. / 2. * lmua * z * pow(NQCD, -1)) + 1. / 2. * lmua * z * NQCD;
    return res;
}

double D0_RG_000(double x, double z, double NF) {
    double res = 0.0;
    res = 1. / 2. * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * pow(z, -1) * NQCD) + (- 1. / 2. * pow(NQCD, -1)) + 1. / 2. * NQCD + 1. / 4. * z * pow(NQCD, -1) + (- 1. / 4. * z * NQCD);
    return res;
}
...
double RG_RG_000(double x, double z, double NF) {
    ...
    if (z != x && z != 1. - x) {
        double tmp = 0.0;
        tmp = (-1. / 4. * pow(z, -1) * pow(NQCD, -1)) + 1. / 4. * pow(z, -1) * NQCD + 1. / 2. * pow(NQCD, -1) + (- 1. / 2. * NQCD) + (- 1. / 4. * x * pow(z, -1) * pow(NQCD, -1)) + 1. / 4. * x * pow(z, -1) * NQCD + 1. / 2. * x * pow(NQCD, -1) + (- 1. / 2. * x * NQCD) + (- 1. / 2. * x * z * pow(NQCD, -1)) + 1. / 2. * x * z * NQCD;
        res += tmp;
    }
