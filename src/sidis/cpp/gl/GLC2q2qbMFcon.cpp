...
double RG_RG_000(double x, double z, double NF) {
    ...
    if (z != x && z != 1. - x) {
        double tmp = 0.0;
        tmp = 3 + (- 3 * pow(NQCD, -2)) + 3 * z * pow(NQCD, -2) + (- 3 * z) + 3 * x * pow(NQCD, -2) + (- 3 * x) + (- 3 * x * z * pow(NQCD, -2)) + 3 * x * z + 1. / 6. * pow(pi, 2) * x * pow(NQCD, -2) + (- 1. / 6. * pow(pi, 2) * x) + (- 1. / 6. * pow(pi, 2) * x * z * pow(NQCD, -2)) + 1. / 6. * pow(pi, 2) * x * z + ln(x) + (- ln(x) * pow(NQCD, -2)) + ln(x) * z * pow(NQCD, -2) + (- ln(x) * z) + (- ln(x) * x * pow(NQCD, -2)) + ln(x) * x + ln(x) * x * z * pow(NQCD, -2) + (- ln(x) * x * z) + (- ln(x) * ln(1 - x) * x * pow(NQCD, -2)) + ln(x) * ln(1 - x) * x + ln(x) * ln(1 - x) * x * z * pow(NQCD, -2) + (- ln(x) * ln(1 - x) * x * z) + 1. / 2. * pow(ln(x), 2) * x * pow(NQCD, -2) + (- 1. / 2. * pow(ln(x), 2) * x) + (- 1. / 2. * pow(ln(x), 2) * x * z * pow(NQCD, -2)) + 1. / 2. * pow(ln(x), 2) * x * z + (- Li2(x) * x * pow(NQCD, -2)) + Li2(x) * x + Li2(x) * x * z * pow(NQCD, -2) + (- Li2(x) * x * z);
        res += tmp;
    }
