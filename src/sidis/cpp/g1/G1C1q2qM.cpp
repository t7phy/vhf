...
double D0_DL_010(double x, double z, double NF) {
    double res = 0.0;
    res = 2 * lmuf * pow(NQCD, -1) + (- 2 * lmuf * NQCD);
    return res;
}
...
double DL_RG_000(double x, double z, double NF) {
    double res = 0.0;
    res = (-1. / 2. * pow(NQCD, -1)) + 1. / 2. * NQCD + 1. / 2. * z * pow(NQCD, -1) + (- 1. / 2. * z * NQCD) + 1. / 2. * ln(1 - z) * pow(NQCD, -1) + (- 1. / 2. * ln(1 - z) * NQCD) + 1. / 2. * ln(1 - z) * z * pow(NQCD, -1) + (- 1. / 2. * ln(1 - z) * z * NQCD) + 1. / 2. * ln(z) * pow(NQCD, -1) + (- 1. / 2. * ln(z) * NQCD) + 1. / 2. * ln(z) * z * pow(NQCD, -1) + (- 1. / 2. * ln(z) * z * NQCD) + (- 1 / (1 - z) * ln(z) * pow(NQCD, -1)) + 1 / (1 - z) * ln(z) * NQCD;
    return res;
}

double DL_RG_001(double x, double z, double NF) {
    double res = 0.0;
    res = (- lmua * pow(NQCD, -1)) + lmua * NQCD + (- lmua * z * pow(NQCD, -1)) + lmua * z * NQCD;
    return res;
}

double D0_RG_000(double x, double z, double NF) {
    double res = 0.0;
    res = 1. / 2. * pow(NQCD, -1) + (- 1. / 2. * NQCD) + 1. / 2. * z * pow(NQCD, -1) + (- 1. / 2. * z * NQCD);
    return res;
}

double RG_DL_000(double x, double z, double NF) {
    double res = 0.0;
    res = (-1. / 2. * pow(NQCD, -1)) + 1. / 2. * NQCD + 1. / 2. * x * pow(NQCD, -1) + (- 1. / 2. * x * NQCD) + 1. / 2. * ln(1 - x) * pow(NQCD, -1) + (- 1. / 2. * ln(1 - x) * NQCD) + 1. / 2. * ln(1 - x) * x * pow(NQCD, -1) + (- 1. / 2. * ln(1 - x) * x * NQCD) + (- 1. / 2. * ln(x) * pow(NQCD, -1)) + 1. / 2. * ln(x) * NQCD + (- 1. / 2. * ln(x) * x * pow(NQCD, -1)) + 1. / 2. * ln(x) * x * NQCD + 1 / (1 - x) * ln(x) * pow(NQCD, -1) + (- 1 / (1 - x) * ln(x) * NQCD);
    return res;
}

double RG_DL_010(double x, double z, double NF) {
    double res = 0.0;
    res = (- lmuf * pow(NQCD, -1)) + lmuf * NQCD + (- lmuf * x * pow(NQCD, -1)) + lmuf * x * NQCD;
    return res;
}

double RG_D0_000(double x, double z, double NF) {
    double res = 0.0;
    res = 1. / 2. * pow(NQCD, -1) + (- 1. / 2. * NQCD) + 1. / 2. * x * pow(NQCD, -1) + (- 1. / 2. * x * NQCD);
    return res;
}
...
double RG_RG_000(double x, double z, double NF) {
    ...
    if (z != x && z != 1. - x) {
        double tmp = 0.0;
        tmp = (-z * pow(NQCD, -1)) + z * NQCD + (- x * pow(NQCD, -1)) + x * NQCD;
        res += tmp;
    }
