...
double RG_RG_000(double x, double z, double NF) {
    ...
    if (z != x && z != 1. - x) {
        double tmp = 0.0;
        tmp = (-6) + 6 * pow(NQCD, -2) + (- 6 * z * pow(NQCD, -2)) + 6 * z + (- 6 * x * pow(NQCD, -2)) + 6 * x + 6 * x * z * pow(NQCD, -2) + (- 6 * x * z) + (- 1. / 3. * pow(pi, 2) * x * pow(NQCD, -2)) + 1. / 3. * pow(pi, 2) * x + 1. / 3. * pow(pi, 2) * x * z * pow(NQCD, -2) + (- 1. / 3. * pow(pi, 2) * x * z) + (- 2 * ln(x)) + 2 * ln(x) * pow(NQCD, -2) + (- 2 * ln(x) * z * pow(NQCD, -2)) + 2 * ln(x) * z + 2 * ln(x) * x * pow(NQCD, -2) + (- 2 * ln(x) * x) + (- 2 * ln(x) * x * z * pow(NQCD, -2)) + 2 * ln(x) * x * z + 2 * ln(x) * ln(1 - x) * x * pow(NQCD, -2) + (- 2 * ln(x) * ln(1 - x) * x) + (- 2 * ln(x) * ln(1 - x) * x * z * pow(NQCD, -2)) + 2 * ln(x) * ln(1 - x) * x * z + (- pow(ln(x), 2) * x * pow(NQCD, -2)) + pow(ln(x), 2) * x + pow(ln(x), 2) * x * z * pow(NQCD, -2) + (- pow(ln(x), 2) * x * z) + 2 * Li2(x) * x * pow(NQCD, -2) + (- 2 * Li2(x) * x) + (- 2 * Li2(x) * x * z * pow(NQCD, -2)) + 2 * Li2(x) * x * z;
        res += tmp;
    }
