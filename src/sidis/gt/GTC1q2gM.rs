...
fn DL_RG_000(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = 1. / 4. * z * pow(NQCD, -1) + (- 1. / 4. * z * NQCD) + 1. / 2. * ln(1.0 - z) * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * ln(1.0 - z) * pow(z, -1) * NQCD) + (- 1. / 2. * ln(1.0 - z) * pow(NQCD, -1)) + 1. / 2. * ln(1.0 - z) * NQCD + 1. / 4. * ln(1.0 - z) * z * pow(NQCD, -1) + (- 1. / 4. * ln(1.0 - z) * z * NQCD) + 1. / 2. * ln(z) * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * ln(z) * pow(z, -1) * NQCD) + (- 1. / 2. * ln(z) * pow(NQCD, -1)) + 1. / 2. * ln(z) * NQCD + 1. / 4. * ln(z) * z * pow(NQCD, -1) + (- 1. / 4. * ln(z) * z * NQCD);
    return res;
}

fn DL_RG_001(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = (- lmua * pow(z, -1) * pow(NQCD, -1)) + lmua * pow(z, -1) * NQCD + lmua * pow(NQCD, -1) + (- lmua * NQCD) + (- 1. / 2. * lmua * z * pow(NQCD, -1)) + 1. / 2. * lmua * z * NQCD;
    return res;
}

fn D0_RG_000(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = 1. / 2. * pow(z, -1) * pow(NQCD, -1) + (- 1. / 2. * pow(z, -1) * NQCD) + (- 1. / 2. * pow(NQCD, -1)) + 1. / 2. * NQCD + 1. / 4. * z * pow(NQCD, -1) + (- 1. / 4. * z * NQCD);
    return res;
}
...
fn RG_RG_000(x: f64, z: f64, NF: f64) -> f64 {
    ...
    if z != x && z != 1. - x {
        let tmp: f64 = (-1. / 4. * pow(z, -1) * pow(NQCD, -1)) + 1. / 4. * pow(z, -1) * NQCD + 1. / 2. * pow(NQCD, -1) + (- 1. / 2. * NQCD) + (- 1. / 4. * x * pow(z, -1) * pow(NQCD, -1)) + 1. / 4. * x * pow(z, -1) * NQCD + 1. / 2. * x * pow(NQCD, -1) + (- 1. / 2. * x * NQCD) + (- 1. / 2. * x * z * pow(NQCD, -1)) + 1. / 2. * x * z * NQCD;
        res += tmp;
    }
