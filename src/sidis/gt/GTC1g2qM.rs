...
fn RG_DL_000(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = (-1. / 2.) + 1. / 2. * x + 1. / 4. * ln(1.0 - x) + (- 1. / 2. * ln(1.0 - x) * x) + (- 1. / 4. * ln(x)) + 1. / 2. * ln(x) * x;
    return res;
}

fn RG_DL_010(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = (- 1. / 2. * lmuf) + lmuf * x;
    return res;
}

fn RG_D0_000(x: f64, z: f64, NF: f64) -> f64 {
    let res: f64 = 1. / 4. + (- 1. / 2. * x);
    return res;
}
...
fn RG_RG_000(x: f64, z: f64, NF: f64) -> f64 {
    ...
    if z != x && z != 1. - x {
        let tmp: f64 = (-1. / 4. * pow(z, -1)) + 1. / 2. * x * pow(z, -1);
        res += tmp;
    }
...
mkcoeff!(
    ("000", [RG_RG_000, RG_D0_000, _, _, _, RG_DL_000, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _]),
    ("010", [_, _, _, _, _, RG_DL_010, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _])
);

/*
  SV mapping:
  - 000: RG_D0, RG_DL, RG_RG
    - 010: RG_DL
*/
