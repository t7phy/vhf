...
def cl_nnlo_q2qpb_es(x, z, orders, rsl):
    ...
    if orders == '000':
        if rsl == 'rr':
            result = 4./3.*pow(x,-1)*z*pow(pi,2)*CF*pow(opx,-1) - 4./3.*pow(x,-1)*z*pow(pi,2)*CF - 8./3.*pow(\
      x,-1)*pow(z,2)*pow(pi,2)*CF*pow(opx,-1) + 8./3.*pow(x,-1)*pow(z,2)*pow(pi,2)*CF - 2*CF + 2*z*\
      CF + 4./3.*z*pow(pi,2)*CF*pow(opx,-1) - 8./3.*pow(z,2)*pow(pi,2)*CF*pow(opx,-1) + 2*x*CF + 8*\
      x*sqrtxz1*rln2*CF - 2*x*z*CF + 4./3.*x*z*pow(pi,2)*CF + 32*x*pow(z,2)*pow(rln2,2)*CF - 8./3.*\
      x*pow(z,2)*pow(pi,2)*CF + 2*ArcTan(z*sqrtxz3)*ln(z*sqrtxz3)*pow(x,-1)*z*sqrtxz3*CF - 2*\
