...
def ct_nnlo_q2qp_es(x, z, orders, rsl):
    ...
    if orders == '000':
        if rsl == 'rr':
            result = 0
            if z != x and z != 1.-x:
                result += 1./3.*pow(x,-2)*pow(pi,2)*CF*pow(opx,-1) - 1./3.*pow(x,-2)*pow(pi,2)*CF - 2./3.*pow(x,-2)\
          *z*pow(pi,2)*CF*pow(opx,-1) + 2./3.*pow(x,-2)*z*pow(pi,2)*CF + 4./3.*pow(x,-2)*pow(z,2)*pow(\
          pi,2)*CF*pow(opx,-1) - 4./3.*pow(x,-2)*pow(z,2)*pow(pi,2)*CF + 1./3.*pow(x,-1)*pow(pi,2)*CF\
           - 2./3.*pow(x,-1)*z*pow(pi,2)*CF + 4./3.*pow(x,-1)*pow(z,2)*pow(pi,2)*CF*pow(opx,-1) + 6*CF\
           + 6*pow(rln2,2)*CF*pow(omx,-1) + 6*sqrtxz1*rln2*CF*pow(omx,-1) - 5./6.*pow(pi,2)*CF*pow(\
