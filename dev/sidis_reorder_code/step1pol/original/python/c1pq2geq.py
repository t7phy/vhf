...
def C1Pq2gEq_DR0123_scheme(inx: float, inz: float, cx: str, cz: str, order: str, ndecimals=ndecimals, LMUR=LMUR, LMUF=LMUF, LMUA=LMUA):
    res = 0.0
    ...
    if cx == "D" and cz == "R":
        ...
        omz = 1.0 - z
        opz = 1.0 + z
        res = -2 * pow(z, -1) * LMUA * CF + 2 * LMUA * CF + z * CF - z * LMUA * CF + 2 * ln(z) * pow(z, -1) * CF - 2 * ln(z) * CF + ln(z) * z * CF + 2 * ln(omz) * pow(z, -1) * CF - 2 * ln(omz) * CF + ln(omz) * z * CF
...
def c1p_q2g_eq(x, z, rsl, order, f=C1Pq2gEq_DR0123_scheme):
    if rsl == "ll":
