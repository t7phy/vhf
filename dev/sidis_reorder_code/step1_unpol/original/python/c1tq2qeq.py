...
def C1Tq2qEq_DR0123_scheme(inx: float, inz: float, cx: str, cz: str, order: str, ndecimals=ndecimals, LMUR=LMUR, LMUF=LMUF, LMUA=LMUA):
    res = 0.0

    if cx == "D" and cz == "D":

        res = -8 * CF - 3.0 / 2.0 * LMUF * CF - 3.0 / 2.0 * LMUA * CF
    ...
    if cx == "D" and cz == "R":
        ...
        omz = 1.0 - z
        opz = 1.0 + z
        res = CF - z * CF + LMUA * CF + LMUA * z * CF + 2 * ln(z) * CF * pow(omz, -1) - ln(z) * CF - ln(z) * z * CF - ln(omz) * CF - ln(omz) * z * CF
    ...
    if cx == "R" and cz == "D":
        ...
        res = CF - x * CF + LMUF * CF + LMUF * x * CF - 2 * ln(x) * CF * pow(omx, -1) + ln(x) * CF + ln(x) * x * CF - ln(omx) * CF - ln(omx) * x * CF
...
def c1tq2q_eq(x, z, rsl, order, f=C1Tq2qEq_DR0123_scheme):
    if rsl == "ll":
