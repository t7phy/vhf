...
def C2Tq2qpEqp_DR0123_scheme(inx: float, inz: float, cx: str, cz: str, order: str, ndecimals=ndecimals, LMUR=LMUR, LMUF=LMUF, LMUA=LMUA):
    ...
    if cx == "R" and cz == "D":
        ...
        res += -1.0 / 4.0 * pow(ln(omx), 2) * x * CF - 1.0 / 3.0 * pow(ln(omx), 2) * pow(x, 2) * CF - ln(omx) * Li2(x) * CF - ln(omx) * Li2(x) * x * CF - Li3(1 - x) * CF - Li3(1 - x) * x * CF - 2.0 / 3.0 * Li2(x) * pow(x, -1) * CF - 3.0 / 2.0 * Li2(x) * CF - 5.0 / 2.0 * Li2(x) * x * CF - 4.0 / 3.0 * Li2(x) * pow(x, 2) * CF + Li2(x) * LMUF * CF + Li2(x) * LMUF * x * CF
    ...
    if cx == "R" and cz == "R":
        ...
        if round(z, ndecimals) != round(x, ndecimals) and round(z, ndecimals) != round(1.0 - x, ndecimals):
            ...
            tmp += (
                ...
                - 19.0 / 12.0 * ln(z) * pow(x, -1) * CF
                + 1.0 / 2.0 * ln(z) * pow(z, -1) * CF
                + 1.0 / 4.0 * ln(z) * pow(poly2, -1) * CF
                - ln(z) * CF * pow(omz, -1)
                - 5.0 / 4.0 * ln(z) * CF
                + 5 * ln(z) * z * CF
                - 1.0 / 2.0 * ln(z) * x * pow(z, -1) * CF
                + ln(z) * x * CF * pow(omz, -1)
                + 5.0 / 4.0 * ln(z) * x * CF
                - 5 * ln(z) * x * z * CF
...
def c2tq2qp__eqp(x, z, rsl, order, f=C2Tq2qpEqp_DR0123_scheme):
    if rsl == "ll":
