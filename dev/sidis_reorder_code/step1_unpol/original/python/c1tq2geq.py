...
def C1Tq2gEq_DR0123_scheme(inx: float, inz: float, cx: str, cz: str, order: str, ndecimals=ndecimals, LMUR=LMUR, LMUF=LMUF, LMUA=LMUA):
    res = 0.0
    ...
    if cx == "D" and cz == "R":
        ...
        omz = 1.0 - z
        opz = 1.0 + z
        res = z * CF - 2 * LMUA * pow(z, -1) * CF + 2 * LMUA * CF - LMUA * z * CF + 2 * ln(z) * pow(z, -1) * CF - 2 * ln(z) * CF + ln(z) * z * CF + 2 * ln(omz) * pow(z, -1) * CF - 2 * ln(omz) * CF + ln(omz) * z * CF
    ...
    if cx == "R" and cz == "R":
        ...
        if round(z, ndecimals) != round(x, ndecimals) and round(z, ndecimals) != round(1.0 - x, ndecimals):
            ...
            tmp = 0.0
            tmp = -pow(z, -1) * CF + 2 * CF - x * pow(z, -1) * CF + 2 * x * CF - 2 * x * z * CF
...
def c1tq2g_eq(x, z, rsl, order, f=C1Tq2gEq_DR0123_scheme):
    if rsl == "ll":
