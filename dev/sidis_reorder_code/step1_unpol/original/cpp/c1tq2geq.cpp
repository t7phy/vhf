double C1Tq2gEq(double inx, double inz, std::string cx, std::string cz,
                double Q, double muR, double muF, double muA)
{
  ...
  if (cx == "D" && cz == "R")
  {
    ...
    double omz = 1. - z;
    double opz = 1. + z;
    res = z * CF - 2 * LMUA * pow(z, -1) * CF + 2 * LMUA * CF - LMUA * z * CF + 2 * ln(z) * pow(z, -1) * CF - 2 * ln(z) * CF +
          ln(z) * z * CF + 2 * ln(omz) * pow(z, -1) * CF - 2 * ln(omz) * CF + ln(omz) * z * CF;
