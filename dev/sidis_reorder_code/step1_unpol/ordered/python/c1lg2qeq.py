...
def C1Lg2qEq_DR0123_scheme(inx: float, inz: float, cx: str, cz: str, order: str, ndecimals=ndecimals, LMUR=LMUR, LMUF=LMUF, LMUA=LMUA):
    res = 0.0
    ...
    if cx == "R" and cz == "R":
        ...
        if round(z, ndecimals) != round(x, ndecimals) and round(z, ndecimals) != round(1.0 - x, ndecimals):
            ...
            tmp = 0
            if "000" == order:
                tmp += 8 * x * TR + (-1) * 8 * pow(x, 2) * TR + 0
            res += tmp

        return res


def c1lg2q_eq(x, z, rsl, order, f=C1Lg2qEq_DR0123_scheme):
    if rsl == "ll":
