...
class conv2:
    ...
    def integrate_pair(self, i, j):
        ...
        if max_xmax <= self.x or max_zmax <= self.z:
            ...
            return (i, j, res_rr + res_rs + res_rl + res_sr + res_ss + res_sl + res_lr + res_ls, err_rr + err_rs + err_rl + err_sr + err_ss + err_sl + err_lr + err_ls)



    def integrator(self):
        # Determine the size of the lists
        size_z = len(self.basis_functions_z)
        size_x = len(self.basis_functions_x)
        
        # Initialize res and err with zeros
        res = [[0] * size_x for _ in range(size_z)]
        err = [[0] * size_x for _ in range(size_z)]
        
        # create list of pairs
        pairs = [(i, j) for i in range(size_z) for j in range(size_x)]

        MAX_CPUS=32
        
        with futures.ProcessPoolExecutor(max_workers=MAX_CPUS) as executor:
            ...
            while jobs_left:
                for pair in pairs_iter:
                    job = executor.submit(self.integrate_pair, pair[0], pair[1])
                    jobs[job] = pair
                    if len(jobs) > MAX_CPUS:
                ...
                for job in futures.as_completed(jobs):
                    ...
                    i, j, res_val, err_val = job.result()
                    res[i][j] = res_val
                    err[i][j] = err_val
