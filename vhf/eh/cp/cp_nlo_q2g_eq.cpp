...
double cp_nlo_q2g_eq(double x, double z, double Q, int rsl, int orders) {
    ...
    if (rsl == 77) {
        ...
        if (std::abs(v) <= tiny && u >= 2 - tiny) {
            ...
            double pt0 = cp_nlo_q2g_eq(x0, z0, Q, rsl, orders);
            double pt1 = cp_nlo_q2g_eq(x1, z1, Q, rsl, orders);
            double pt2 = cp_nlo_q2g_eq(x2, z2, Q, rsl, orders);
            ...
            if (z != x && z != 1. - x) {
                result += -pow(z, -1) * CF + 2 * CF - 2 * z * CF - x * pow(z, -1) * CF + 2 * x * CF;
            }
            if (z < 1. - x && z < x) {
            }
            if (z > 1. - x && z < x) {
